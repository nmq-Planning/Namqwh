...
// Some workbooks have a title/cover row (e.g. a report name) above the real
// column headers. Scan the first several rows and use whichever one has the
// most cells matching a known header (Item Code, Description, etc.) as the
// actual header row, instead of always assuming it's row 1.
function findHeaderRowIndex(sheet) {
  ...
  for (let i = 0; i < maxScan; i++) {
    const row = raw[i] || [];
    const score = row.filter((cell) => EXCEL_HEADER_ALIASES[normalizeHeader(cell)]).length;
    if (score > bestScore) {
      bestScore = score;
      bestIndex = i;
    }
...
/* ---------- Local cache of the last synced items (offline fallback) ---------- */
